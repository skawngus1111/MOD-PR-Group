...
def init_net(net, init_type='normal', init_gain=0.2, gpu_id='cuda:0') :
    net.to(gpu_id)
...
class Inconv(nn.Module) :
    def __init__(self, in_ch, out_ch, norm_layer, use_bias):
        ...
        self.inconv = nn.Sequential(
            nn.ReflectionPad2d(3), # 입력 경계의 반사를 사용하여 입력 텐서를 패딩
            nn.Conv2d(in_ch, out_ch, kernel_size=(7, 7), padding=(0, 0), bias=use_bias),
            norm_layer(out_ch),
            nn.ReLU(True)
        )
...
class Down(nn.Module) :
    def __init__(self, in_ch, out_ch, norm_layer, use_bias):
        ...
        self.down = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=use_bias),
            norm_layer(out_ch),
            nn.ReLU(True)
        )
...
class ResBlock(nn.Module) :
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [nn.Conv2d(dim, dim, kernel_size=(3, 3), padding=(p, p), bias=use_bias),
                       norm_layer(dim),
                       nn.ReLU(True)]
...
class Up(nn.Module) :
    def __init__(self, in_ch, out_ch, norm_layer, use_bias):
        ...
        self.up = nn.Sequential(
            nn.ConvTranspose2d(in_ch, out_ch, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), output_padding=(1, 1), bias=use_bias),
            norm_layer(out_ch),
            nn.ReLU(True)
        )
...
class Outconv(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.outconv = nn.Sequential(
            ...
            nn.Conv2d(in_ch, out_ch, kernel_size=(7, 7), padding=(0, 0)),
            nn.Tanh()
        )
...
class NLayerDiscriminator(nn.Module) :
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False):
        ...
        for n in range(1, n_layers) :
            ...
            sequence += [
                nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=(kw, kw), stride=(2, 2), padding=(padw, padw), bias=use_bias),
        ...
        sequence += [
            nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult,
                      kernel_size=(kw, kw), stride=(1, 1), padding=(padw, padw), bias=use_bias),
            norm_layer(ndf * nf_mult),
            nn.LeakyReLU(0.2, True)
        ]

        sequence += [nn.Conv2d(ndf * nf_mult, 1, kernel_size=(kw, kw), stride=(1, 1), padding=(padw, padw))]
...
class GANLoss(nn.Module) :
    def __init__(self, use_lsgan=True, target_real_label=1.0, target_fake_label=0.0):
        ...
        # register_buffer() : 일반적으로 모델 매개 변수로 간주되지 않는 버퍼를 등록하는 데 사용된다.
        # 예를 들어 BatchNorm2d에서 "running_mean"은 매개 변수는 아니지만 상태를 표현하는 데 사용될 수 있다.
        ...
        if use_lsgan : # LSGAN : Least Square GAN
